...
<head>
    ...
    <title>{$Think.const.WEB_NAME}预约中心</title>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="breadcrumbs mgl2 pd-space">
                            <ul>
                                <li><a>{$Think.const.WEB_NAME}</a></li>
                                <li>个人中心</li>
                            </ul>
                        </div><!-- breadcrumb -->
                        ...
                        <div class="sidebar">
                            <div class="widget widget-nav-menu style2">
                                <ul class="widget-menu">
                                    <li><a href="/index/index/setting">修改头像</a></li>
                                    <li><a href="/index/index/setting_password">修改密码</a></li>
                                    <li><a href="/index/index/setting_user">个人信息</a></li>
                                    <li><a href="/index/index/setting_order">我的订单</a></li>
                                    <li   class="active"><a href="/index/index/setting_beautyorder">我的预约</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-9 col-sm-8">
					 ...
					 <th>预约时间</th>
                                      ...
                                      <tbody>
                                        ...
                                        {foreach $beautyorder_info as $key => $value}
                                        <tr>
                                          <td>
						{$value.id}<input  type="hidden" value="{$value.id}" id="orderid{$value.id}">

					 </td>
                                          <td>{$value.bname}</td>
                                          <td>{$value.deposit}</td>
                                          <td>{$value.payment}</td>
					 <td>{:date('Y-m-d H:i', $value.book_time)}</td>
                                          <td>
						{if ($value.status == '未付定金')}
						<form action="/alipay/pagepay/pagepay.php" method="post" class="form-inline">
							<!-- 付款金额 -->
                                        			...
                                        			<input type="submit" name="dosubmit" id="book{$value.id}" value="支付定金" />
						<script>
        							var oBid{$value.id} = document.getElementById('orderid{$value.id}');
        							var oBook{$value.id} = document.getElementById('book{$value.id}');
        							oBook{$value.id}.onclick = function ()
        							{
            							var xhr = new XMLHttpRequest();
            							xhr.open('post', '/index/index/book_pay', false);
            							xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
            							xhr.send('id=' + oBid{$value.id}.value);
        							}
						...
						{/if}
                                            ...
                                            <form action="/index/index/setting_beautyorder" method="post">
                                          <td>
					     <input type="submit" class="btn-default" name="delete" value="删除订单">
